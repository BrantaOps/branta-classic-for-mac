...
class Verify: BackgroundAutomation {
    ...
    override class func run() {
        ...
        Timer.scheduledTimer(withTimeInterval: Branta.Constants.Misc.API_CADENCE, repeats: true) { _ in
            Bridge.fetchLatest { _ in }
        }
        
        timer?.invalidate()
        let cadence = Settings.readFromDefaults()[Branta.Constants.Prefs.SCAN_CADENCE] as! Double
...
extension Verify {
    ...
    static func matchSignatures(wallets: [CrawledWallet]) -> [CrawledWallet] {
        let architectureSpecificHashes: Branta.Constants.Alias.RuntimeHashType = Bridge.getRuntimeHashes()
                ...
                if alreadyWarned[wallet.fullWalletName] == false && !appDelegate!.foreground {
                    ...
                    if !(wallet.notFound) {
                        ...
                        appDelegate?.notificationManager?.showNotification(
                            title: titleString,
                            body: bodyString,
                            key: Branta.Constants.Notify.UPON_STATUS_CHANGE
                        )
    ...
    static func crawl() -> [CrawledWallet] {
        ...
        do {
            ...
            for item in items {
                if Branta.Constants.Wallets.NAMES.contains(item) {
                    ...
                    for brantaVersion in versions {
                        do {
                            ...
                            BrantaLogger.log(s: "WalletViewController#showDetails error: \(error)")
                        }
                    }

                    let crawledWallet = CrawledWallet(
                        cls: BrantaWallet.getCls(forStr: item),
                        fullWalletName: item,
            ...
            for target in Branta.Constants.Wallets.NAMES {
